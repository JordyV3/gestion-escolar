                    ...
                    <div class="box-tools">
                        ...
                        <?php if(($siteinfos->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1)) { ?>
                            <?php if (permissionChecker('activities_delete') && ($usertypeID == 1 || ($usertypeID == $activity->usertypeID && $userID == $activity->userID))) { ?>
                                <a onclick="return confirm('Estás a punto de borrar un registro Esto no se puede deshacer. ¿Estás seguro?')" class="btn btn-box-tool" href="<?=base_url('activities/delete/'.$activity->activitiesID)?>"><i class="fa fa-times"></i></a>
                            <?php } ?>
                        <?php } ?>
                    </div>
                </div>
                            ...
                            <div class="box-comment">
                                ...
                                <div class="comment-text">
                                    <span class="username">
                                        ...
                                        <?php if(($siteinfos->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1)) { ?>
                                            <?php if (permissionChecker('activities_delete') && ($usertypeID == 1 || ($usertypeID == $activity->usertypeID && $userID == $activity->userID))) { ?>
                                                <a href="<?=base_url('activities/delete_comment/'.$comment->activitiescommentID)?>" onclick="return confirm('Estás a punto de borrar un registro Esto no se puede deshacer. ¿Estás seguro?')" style="margin-left: 5px; margin-top: -4px; font-size: 15px;" class="text-muted pull-right text-danger"><i class="fa fa-trash"></i></a> &nbsp;&nbsp;&nbsp;
                                            <?php } ?>
                                        <?php } ?>
